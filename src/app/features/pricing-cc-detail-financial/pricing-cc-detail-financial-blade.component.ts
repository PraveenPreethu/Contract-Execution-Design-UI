...
import { PricingCcDetailFinancial, PricingCcDetailFinancialDelta, ProductLookup, FixedFloatLookup } from '../../core/models';
import { ConfirmDialogComponent } from '../../shared/components/confirm-dialog/confirm-dialog.component';
import { PricingCcDetailFinancialDeltaBladeComponent } from '../pricing-cc-detail-financial-delta/pricing-cc-detail-financial-delta-blade.component';

@Component({
  selector: 'app-pricing-cc-detail-financial-blade',
  standalone: true,
  imports: [
  ...
  templateUrl: './pricing-cc-detail-financial-blade.component.html',
  styleUrl:    './pricing-cc-detail-financial-blade.component.scss'
})
export class PricingCcDetailFinancialBladeComponent implements OnInit {
  ...
  private svc       = inject(PricingCcDetailFinancialService);
  private deltaSvc  = inject(PricingCcDetailFinancialDeltaService);
  ...
  saving      = signal(false);
  deltas      = signal<PricingCcDetailFinancialDelta[]>([]);
  products    = signal<ProductLookup[]>([]);
  fixedFloats = signal<FixedFloatLookup[]>([]);

  deltaCols = ['id', 'pricingDate', 'price', 'fixedFloat', 'curveBumped', 'bumpedPrice', 'delta', 'actions'];
  ...
  save(): void {
    ...
    const payload: PricingCcDetailFinancial = { id: this.id || 0, ...this.form.getRawValue(), lastUpdateDate: new Date().toISOString() };
  ...
  openDelta(d?: PricingCcDetailFinancialDelta): void {
    this.bladeSvc.open(PricingCcDetailFinancialDeltaBladeComponent,
      d ? `Financial Delta — #${d.id}` : 'New Financial Delta',
  ...
  confirmDeleteDelta(d: PricingCcDetailFinancialDelta, event: Event): void {
    event.stopPropagation();
    this.dialog.open(ConfirmDialogComponent, {
      data: { title: 'Delete Delta', message: `Delete financial delta #${d.id}?`, action: 'Delete', danger: true }
